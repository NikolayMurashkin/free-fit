(() => {
  ...
  const d = document.querySelectorAll(".items-events__card-ibg"),
    p = document.querySelector(".items-events");
  console.log(document.documentElement.clientWidth),
    d.length >= 4 && (p.style.flexWrap = "wrap"),
    (window.FLS = !0),
    (function (t) {
      let e = new Image();
      (e.onload = e.onerror =
    ...
    (function () {
      const t = document.querySelectorAll("._anim-items");
      if (t.length > 0) {
        function e(e) {
          for (let e = 0; e < t.length; e++) {
            const s = t[e],
              ...
              i = o(s).top,
              a = 4;
            let r = window.innerHeight - n / a;
            n > window.innerHeight &&
              (r = window.innerHeight - window.innerHeight / a),
              window.pageYOffset > i - r && window.pageYOffset < i + n
                ? s.classList.add("_scrolled")
                : s.classList.contains("_anim-no-hide") ||
                  s.classList.remove("_scrolled");
          }
        }
        function o(t) {
